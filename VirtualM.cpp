...
int main_() {
    ...
    //s0->DisplayDetails() - > shape -> NU MERGE
    //s1->DisplayDetails() - > detalii pt cerc
   
    s1 = new Circle("Cerc test", 10);
    s1->DisplayDetails();
    cout << "Aria:" << s1->CalculateArea() << endl << "------------------------" << endl;
    //s2->DisplayDetails() - > rectangle

    s2 = new Rectangle("rectangle Test", 10, 5);
    s2->DisplayDetails();
    cout << "Aria:" << s2->CalculateArea() << endl << "------------------------" << endl;
